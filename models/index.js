...
router.get('/api/GetUsers', function (request, response) {
    console.log('IN get data from mongo.');
...
var getSaveToMongoSchemaData = function (data, sheet) {
    if (data.length > 0) {
        ...
        Userdetails.collection.insert(mongoData, function (err, docs) {
            if (err) {
                return console.error(err);
            } else {
                console.log("Multiple documents inserted to Collection. Count:-" + mongoData.length);
            }
        });
